...
function PreviewInput({ question, id, tag, dataContext }) {
  ...
  const setEditValue = (id, tag) => {
    ...
    if (currentData && Array.isArray(currentData[tag])) {
      ...
      if (existingQuestion) {
        // If the question with the id exists, set the input value to the existing answer
        setEdit(existingQuestion.edit);
      }
    }
  };

    useEffect(() => {setEditValue(id, tag)}, [dataContext.data]);
  ...
  return (
    <section className="flex flex-col justify-center py-6 md:px-4 tracking-tight mt-3 w-full border border-violet-200 border-solid max-md:px-5 max-md:max-w-full">
      ...
      <div className={`group flex gap-2.5 items-center px-3 py-6 mt-3 w-full text-center border-b-[3px] text-neutral-400 max-md:max-w-full focus-within:border-primary ${edit? "border-violet-200" : "border-primary"}`}>
        <input
          type="text"
          id={id}
